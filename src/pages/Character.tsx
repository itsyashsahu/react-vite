  ...
  return (
    <>
      <Helmet>
        ...
        <link rel="canonical" href={`${Config.FRONTEND_URL}/character/${id}`} />
      </Helmet>
      <Header />
      <section className="flex items-center justify-center flex-1 py-8 antialiased md:py-16 ">
              ...
              <div className="flex flex-col items-center justify-center flex-1 mt-6 sm:mt-8 lg:mt-0">
                <div>
                  ...
                  <p className="mb-6 text-gray-500 dark:text-gray-400">
                    ...
                    hailing from {characterQuery?.data?.data?.origin?.name}{" "}
                    within the Rick and Morty universe.{" "}
                    ...
                    {characterQuery.data?.data?.status?.toLowerCase()} and
                    thriving. Its debut was marked in Episode{" "}
        ...
        {characterQuery.isLoading && <LoadingCard/>}
        {characterQuery.isError && <ErrorCard  errorMessage={characterQuery.error?.['message'] || 'something went wrong'} />}
      </section>
    </>
  );
};
